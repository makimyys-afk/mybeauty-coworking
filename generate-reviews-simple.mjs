...
async function generateReviews() {
  ...
  const [allWorkspaces] = await connection.execute('SELECT * FROM workspaces');
  ...
  for (const workspace of allWorkspaces) {
    ...
    for (let i = 0; i < reviewCount; i++) {
      ...
      await connection.execute(
        'INSERT INTO reviews (workspaceId, userId, rating, comment, createdAt) VALUES (?, ?, ?, ?, ?)',
        [workspace.id, getRandomInt(1, 20), rating, comment, createdAt]
      );
    ...
    await connection.execute(
      ...
      [parseFloat(avgRating), reviewCount, workspace.id]
    );
